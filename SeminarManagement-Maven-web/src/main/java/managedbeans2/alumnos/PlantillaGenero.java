 ...
 * @author xarly
 */
@WebServlet(name = "PlantillaGenero", urlPatterns = {"/DocGenero"})
public class PlantillaGenero extends HttpServlet {

    public String TEMPLATE_LOCATION = "/resources/plantillas/plantilla_genero.docx";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String fileName = "" + "SolicitudGenero_"
     ...
     * @throws Exception
     */
    private void replaceText(ContentAccessor c, String nombre, String fecha)
            throws Exception {
        for (Object p : c.getContent()) {
            if (p instanceof ContentAccessor) {
                replaceText((ContentAccessor) p, nombre, fecha);
            } else if (p instanceof JAXBElement) {
                Object v = ((JAXBElement) p).getValue();

                if (v instanceof ContentAccessor) {
                    replaceText((ContentAccessor) v, nombre, fecha);
                    ...
                    if (text.compareTo("nombreAlumno") == 0) {
                        t.setValue(nombre);
                    } else if (text.compareTo("fechaSolicitud") == 0) {
                        t.setValue(fecha);
                    }
                }
            }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(PlantillaGenero.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(PlantillaGenero.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
