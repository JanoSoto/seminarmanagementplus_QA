...
public class AsignaturaController implements Serializable {
    ...
    */
    ...
    public String getDatosPlan(Versionplan version){
        if(version != null){
            String salida = "";
            salida += version.getPlanestudioId().getCodigo();
            salida += " - " + version.getAnio();
            salida += "." + version.getVersion();
            int j;
            if((j = version.getPlanestudioId().getJornada()) == 0)
                salida += " (Diurno)";
            else
                salida += " (Vespertino)";
            return salida;
    ...
    */
    ...
    public ArrayList<Versionplan> getPlanesDeEstudio(int carreraId){
        ...
        for (Versionplan v : versiones){
            if (!planesEstudio.contains(v) && v.getPlanestudioId().getCarrera().getId().toString().equals(carreraId+"") && v.isPlanificado() && haveAsignaturas(v))
                planesEstudio.add(v);
        }
        return planesEstudio;
    }
    ...
    public List<Versionplan> getAllPlanesDeEstudio(int carreraId){
        ...
        for (Versionplan v : versiones){
            if (!planesEstudio.contains(v) && v.getPlanestudioId().getCarrera().getId().toString().equals(carreraId+""))
                planesEstudio.add(v);
        }
